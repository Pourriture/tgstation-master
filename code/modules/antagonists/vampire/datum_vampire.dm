...
/datum/antagonist/vampire/proc/forge_objectives()
	...
	/// Vampires have two "simple" objectives, an objective to thrall a specific person or have X thralls by the end, and possibly either a hijack objective or a steal AI objective.
	if (prob(50))
		var/datum/objective/steal/steal_objective = new
		steal_objective.owner = owner /// change to team later
		objectives += steal_objective

	/// Thrall objective (wip)
	var/datum/objective/assassinate/thrall_objective = new
	thrall_objective.owner = owner
	objectives += thrall_objective

	/// Hijack objective - may replace with Steal or Slave the AI?
	if (prob(100)))
